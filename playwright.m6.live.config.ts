...
const authorized = process.env.SKETCHYCUT_M6_LIVE_DISPATCH_AUTHORIZED;
...
if (expectedMode !== "live") {
  throw new Error("SKETCHYCUT_M6_LIVE_MODE_REQUIRED");
}
if (protectionBypass === undefined || protectionBypass.length < 20) {
  throw new Error("VERCEL_AUTOMATION_BYPASS_SECRET_REQUIRED");
}
if (authorized !== "1") {
  throw new Error("SKETCHYCUT_M6_LIVE_DISPATCH_AUTHORIZATION_REQUIRED");
}

export default defineConfig({
  testDir: "./tests/m6-live-probe",
  fullyParallel: false,
  forbidOnly: true,
  retries: 0,
  workers: 1,
