...
export class MemoryM6Store implements M6Store {
  readonly #clock: () => number;
  readonly #values = new Map<string, ExpiringValue>();
  readonly #sessions = new Map<string, M6SessionRecord>();
  ...
  readonly #exposureAuthorizations: M61ExposureAuthorizationRecord[] = [];
  #globalExposure: M61GlobalExposureState;

  constructor(clock: () => number = Date.now) {
    this.#clock = clock;
    this.#globalExposure = M61GlobalExposureStateSchema.parse({
      schemaVersion: "1.0",
      authorizedCeilingMicrousd: M6_POLICY.generation.initialGlobalExposureCeilingMicrousd,
  ...
  createSession(recordCandidate: M6SessionRecord, ttlSeconds: number): Promise<void> {
  ...
  #readSession(sessionId: string): M6SessionRecord | null {
    ...
    return M6SessionRecordSchema.parse(structuredClone(record));
  }
  ...
  setLastProject(sessionId: string, projectId: string): Promise<boolean> {
    ...
    this.#sessions.set(sessionId, M6SessionRecordSchema.parse({ ...record, lastProjectId: projectId }));
    return Promise.resolve(true);
  }
    ...
    if (!input.verified) {
      ...
      if (state.count >= input.maximumAttempts) {
        state.blockedUntilMs = Math.max(
          ...
          state.windowStartedAtMs + input.windowMs,
        );
      }
  ...
  reserveGeneration(input: {
    ...
    const next = M6SessionRecordSchema.parse({
      ...session,
  ...
  appendLedgerAttempt(attemptCandidate: LiveCallAttempt): Promise<void> {
    ...
    if (this.#ledger.some((item) => item.attemptId === attempt.attemptId ||
      ...
      return Promise.reject(new Error("M6_LEDGER_DUPLICATE_IDENTITY"));
    }
  ...
  readGlobalExposureState(): Promise<M61GlobalExposureState> {
    return Promise.resolve(M61GlobalExposureStateSchema.parse(structuredClone(this.#globalExposure)));
  }
  ...
  authorizeGlobalExposure(input: {
    ...
    if (this.#exposureAuthorizations.some((item) =>
      ...
      return Promise.resolve(M61ExposureAuthorizationDecisionSchema.parse({
        applied: false,
  ...
  readExposureAuthorizations(): Promise<M61ExposureAuthorizationRecord[]> {
    return Promise.resolve(this.#exposureAuthorizations.map((record) =>
      M61ExposureAuthorizationRecordSchema.parse(structuredClone(record))));
  }
}
