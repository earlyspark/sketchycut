...
import {
  ...
  type M6PersistedProject
} from "./project-persistence.js";

export const M6_PACKAGE_GENERATOR_VERSION = "1.0.0" as const;
export const M6_IMPORT_COMPLEXITY_BUDGET = {
  policy: "within-m5-exact-import-observed-envelope-not-new-import-proof",
  evidenceArtifactSha256: "168e863072aa11f8080d2898312b21bf6b874c7012fd8e118e64b2e36808268d",
...
const PackageSheetSchema = z.object({
  ...
  importComplexityBudget: z.object({
    policy: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.policy),
    evidenceArtifactSha256: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.evidenceArtifactSha256),
    withinObservedEnvelope: z.literal(true)
  }).strict()
}).strict();

export const M6PackageManifestSchema = z.object({
  schemaVersion: z.literal("1.0"),
  packageGeneratorVersion: z.literal(M6_PACKAGE_GENERATOR_VERSION),
  ...
  importComplexityBudget: z.object({
    policy: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.policy),
    evidenceArtifactSha256: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.evidenceArtifactSha256),
    studioDesktopVersion: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.studioDesktopVersion),
    maximumPathCount: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.maximumPathCount),
    maximumSegmentCount: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.maximumSegmentCount),
    maximumVertexCount: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.maximumVertexCount),
    maximumSvgByteSize: z.literal(M6_IMPORT_COMPLEXITY_BUDGET.maximumSvgByteSize)
  }).strict(),
...
export type M6PackageManifest = z.infer<typeof M6PackageManifestSchema>;
export type M6Package = {
  filename: string;
  bytes: Uint8Array;
  sha256: string;
  manifest: M6PackageManifest;
};
...
function instructionMarkdown(compiled: GeneratedCompiledProject): string {
  ...
  if (legend === undefined || instructions === undefined) {
    throw new Error("M6_PACKAGE_LINKED_BUILD_PROJECTIONS_MISSING");
  }
...
function assertPlainFabricationSvg(svg: string): void {
  ...
  if (forbidden.some((pattern) => pattern.test(svg))) {
    throw new Error("M6_PACKAGE_SVG_NOT_PLAIN");
  }
  if (!/<svg\b[^>]*\bwidth="[0-9.]+mm"[^>]*\bheight="[0-9.]+mm"/i.test(svg)) {
    throw new Error("M6_PACKAGE_SVG_MM_ROOT_MISSING");
  }
}
...
function groupManifest(input: {
  ...
  const sheets = input.artifacts.bundle.fabrication.sheets.map((sheet) => {
    ...
    if (svg === undefined) throw new Error("M6_PACKAGE_SVG_MISSING");
    ...
    if (
      complexity.pathCount > M6_IMPORT_COMPLEXITY_BUDGET.maximumPathCount ||
      complexity.segmentCount > M6_IMPORT_COMPLEXITY_BUDGET.maximumSegmentCount ||
      complexity.vertexCount > M6_IMPORT_COMPLEXITY_BUDGET.maximumVertexCount ||
      complexity.svgByteSize > M6_IMPORT_COMPLEXITY_BUDGET.maximumSvgByteSize
    ) {
      throw new Error("M6_PACKAGE_IMPORT_COMPLEXITY_BUDGET_EXCEEDED");
    }
    return PackageSheetSchema.parse({
      ...
      importComplexityBudget: {
        policy: M6_IMPORT_COMPLEXITY_BUDGET.policy,
        evidenceArtifactSha256: M6_IMPORT_COMPLEXITY_BUDGET.evidenceArtifactSha256,
        withinObservedEnvelope: true
      }
    });
  });
  return {
...
function renderCompletePackageChecklist(input: {
  ...
  const lines = [
    ...
    `Schema: ${SCHEMA_VERSION}; kernel generator: ${GENERATOR_VERSION}; package generator: ${M6_PACKAGE_GENERATOR_VERSION}.`,
...
export async function buildM6Package(projectCandidate: M6PersistedProject): Promise<M6Package> {
  const project = M6PersistedProjectSchema.parse(projectCandidate);
  ...
  if (
    ...
    throw new Error("M6_PACKAGE_PROJECT_REVALIDATION_MISMATCH");
  }
  ...
  for (const [document, artifacts] of [
    ...
    if (validateFabricationProjection(artifacts.bundle.fabrication, document.parts).status !== "pass") {
      throw new Error("M6_PACKAGE_FABRICATION_VALIDATION_FAILED");
    }
  }
  ...
  files.set("handoff/xtool-studio-handoff.json", json({
    schemaVersion: "1.0",
    packageGeneratorVersion: M6_PACKAGE_GENERATOR_VERSION,
  ...
  const manifest = M6PackageManifestSchema.parse({
    ...
    studioHandoff,
    importComplexityBudget: M6_IMPORT_COMPLEXITY_BUDGET,
    artifactGroups: groups,
