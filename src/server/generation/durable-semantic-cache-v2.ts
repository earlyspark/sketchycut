...
import {
  ...
  type CachedSemanticValueV2,
  type SemanticCacheResolutionV2,
  type SemanticCacheV2,
  validateCachedSemanticValueV2
...
): Promise<CachedSemanticValueV2 | null> {
  try {
    return await validateCachedSemanticValueV2({ candidate: JSON.parse(source) as unknown, request });
  } catch {
    return null;
  }
}

export class DurableSemanticCacheV2 implements SemanticCacheV2 {
  readonly #store: GenerationStore;
  readonly #inflight = new Map<string, Promise<{ value: CachedSemanticValueV2; dispatched: boolean }>>();
  ...
  async #resolveDistributedMiss(
    ...
    request: SemanticGenerationRequestV2,
    dispatch: (request: SemanticGenerationRequestV2) => Promise<unknown>,
  ): Promise<{ value: CachedSemanticValueV2; dispatched: boolean }> {
    const cacheKey = generationKeys.cache(requestDigest);
    ...
    while (Date.now() < deadline) {
      ...
      if (acquired) {
        try {
          ...
          const value = await validateCachedSemanticValueV2({ candidate: await dispatch(request), request });
