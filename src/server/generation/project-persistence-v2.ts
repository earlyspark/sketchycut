...
import {
  CanonicalGenerationSourceV2Schema,
  currentComponentManifestV2,
  generationOutcomeV2FromPlanner
} from "../../interpretation/generation-outcome-v2.js";
...
async function readStored(store: GenerationStore, projectId: string) {
  ...
  if (typeof candidate === "object" && candidate !== null &&
      "schemaVersion" in candidate && candidate.schemaVersion !== "2.0") {
...
async function replan(input: {
  ...
  if (await hashCanonical(await currentComponentManifestV2()) !== await hashCanonical(input.source.componentManifest)) {
    throw new CurrentProjectError("UNSUPPORTED_PROJECT_VERSION");
  }
  ...
  if (outcome.kind !== "supported" && outcome.kind !== "simplified") throw new CurrentProjectError("INVALID");
...
export async function createCurrentPersistedProject(input: {
  ...
  const record = CurrentPersistedProjectSchema.parse({
    schemaVersion: "2.0", projectId: `project-${randomUUID()}`, ownerSessionId: input.ownerSessionId,
...
export async function updateCurrentPersistedProject(input: {
  ...
  const deterministicControls = GenerationDeterministicControlsV2Schema.parse(input.deterministicControls);
