...
export const M5_REPLAY_SCENARIOS: readonly M5ReplayScenario[] = Object.freeze([
  {
    id: "rigid-structure",
  ...
  {
    id: "invalid-output",
    brief: "Replay an intentionally invalid structured interpretation.",
    behavior: "rigid",
...
export function findM5ReplayScenario(brief: string): M5ReplayScenario | null {
  return M5_REPLAY_SCENARIOS.find((scenario) => scenario.brief === brief) ?? null;
}
...
export function buildM5ReplayIntent(
  request: SemanticGenerationRequestV1,
  scenario: M5ReplayScenario,
): unknown {
  if (scenario.expectedOutcome === "schema-failure") {
    return { schemaVersion: "1.0", title: "Invalid replay output", unknownField: true };
  }
