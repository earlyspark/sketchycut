...
const ACCESS_CODE = "m6-fixture-access";
...
test("cold development keeps the first session and all protected-route state", async ({ page }) => {
  ...
  await expect(page.getByLabel("Replay scenario")).toBeVisible();
  await expect(page.getByLabel(/Maker brief/)).toHaveAttribute("readonly", "");
...
test("development CSP admits the Worker and examples compile without prewarming create", async ({ page }) => {
  const observed = observeNetwork(page);
  const errors: string[] = [];
...
test("invalid fixture preserves its exact brief, reference, and maker-set roles", async ({ page }) => {
  const observed = observeNetwork(page);
  await coldLogin(page);
  await page.getByLabel("Replay scenario").selectOption({ label: "Invalid output (failure-preservation fixture)" });
