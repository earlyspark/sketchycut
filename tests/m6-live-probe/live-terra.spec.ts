...
// One authorized deployed live Terra dispatch through the protected /create
// flow, using the frozen M5 rigid brief and the synthetic sample reference.
...
const OUTCOME_PATH = resolve(
  process.env.SKETCHYCUT_M6_LIVE_OUTCOME_PATH ?? "docs/evidence/m06/live/live-evaluation.json",
);
...
test("performs exactly one deployed live Terra generation", async ({ page }) => {
  ...
  const record = {
    ...
    deploymentUrl: process.env.SKETCHYCUT_M6_DEPLOYMENT_URL,
    brief: RIGID_BRIEF,
    generationRequests,
