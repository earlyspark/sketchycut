...
import { DurableSemanticCache } from "../../src/server/m6/durable-semantic-cache.js";
...
describe("M6 durable exact semantic cache", () => {
  it("survives a cache instance restart without storing the brief or fabrication authority", async () => {
    ...
    const stored = await store.getValue(m6Keys.cache(digest));
