...
async function session(store: UpstashM6Store, id: string, nowMs: number): Promise<void> {
...
describe("M6.1 Upstash atomic scripts", () => {
  ...
  it("atomically appends a reviewed ceiling authorization and rejects stale state", async () => {
    ...
    const review = await reviewM61ExposureIncrease({
      ...
      authorizationId: "m61-upstash-authorization"
    });
    ...
    const stale = await reviewM61ExposureIncrease({
      store,
      evidenceSha256: "b".repeat(64),
      reviewNote: "Stale Upstash proof.",
      authorizationId: "m61-upstash-stale"
    });
    ...
    expect(await applyReviewedM61ExposureIncrease({ store, review: stale })).toMatchObject({
      applied: false,
      reason: "stale-state"
    });
  });
