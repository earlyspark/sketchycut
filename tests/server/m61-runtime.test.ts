...
import { verifyAccessCodeConstantTime } from "../../src/server/m6/access.js";
import { deriveM61RuntimeOrigin } from "../../src/server/m6/runtime-origin.js";
import {
  buildM61DevelopmentEnvironment,
  M61_FIXTURE_ACCESS_CODE
} from "../../tools/m61-development.js";
...
describe("M6.1 runtime modes", () => {
  it("requires an explicit replay-fixture guard outside tests", () => {
    const base = {
    ...
    expect(() => readM6RuntimeConfig(base)).toThrow("M61_CONFIG_REPLAY_FIXTURE_GUARD_MISSING");
    expect(readM6RuntimeConfig({ ...base, SKETCHYCUT_FIXTURE_MODE: "1" }))
      .toMatchObject({ generationExperience: "replay-fixture" });
  });

  it("sanitizes fixture development", () => {
    const fixtures = buildM61DevelopmentEnvironment("fixtures", {
      NODE_ENV: "development",
  ...
  it("admits live development only with durable accounting and every required secret", () => {
    ...
    const live = buildM61DevelopmentEnvironment("live", {
      NODE_ENV: "development",
      SKETCHYCUT_M6_STORE: "memory",
    ...
    const parsedLive = readM6RuntimeConfig(live);
    ...
    expect(() => buildM61DevelopmentEnvironment("live", {
      NODE_ENV: "development",
      SKETCHYCUT_ACCESS_CODE_SHA256: accessCodeDigestHex("local-live"),
      SKETCHYCUT_SESSION_SIGNING_SECRET: signingSecret
    })).toThrow("M6_CONFIG_UPSTASH_REDIS_REST_URL_MISSING");
  });

  it("keeps launcher-only plaintext and deployment metadata shadowed after Next loads env files", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "sketchycut-m61-next-env-"));
    try {
  ...
  it("derives origin only from trusted runtime metadata", () => {
    expect(deriveM61RuntimeOrigin({ NODE_ENV: "development" })).toBe("local-development");
    expect(deriveM61RuntimeOrigin({ NODE_ENV: "production", VERCEL: "1", VERCEL_ENV: "preview" }))
      .toBe("deployment-preview");
    expect(deriveM61RuntimeOrigin({ NODE_ENV: "production", VERCEL: "1", VERCEL_ENV: "production" }))
      .toBe("deployment-production");
    expect(deriveM61RuntimeOrigin({ NODE_ENV: "test", SKETCHYCUT_TEST_MODE: "1", VERCEL: "1" }))
      .toBe("test-recorded");
  });
  ...
  it("retains the development memory store across module reset and re-import", async () => {
    ...
    const first = await import("../../src/server/m6/store.js");
    const sessionId = `m61-cold-${crypto.randomUUID()}`;
    await first.createM6Store(config).createSession({
      schemaVersion: "1.0",
      sessionId,
      issuedAtMs: 1,
    ...
    vi.resetModules();
    const second = await import("../../src/server/m6/store.js");
    expect((await second.createM6Store(config).readSession(sessionId))?.sessionId).toBe(sessionId);
  });
});
