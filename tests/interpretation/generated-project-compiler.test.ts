...
describe("deterministic generated-project compilation", () => {
  it("maps and compiles rigid, revolute, and prismatic fixture semantics through registered graphs", async () => {
  ...
  it("applies dimensions and motif placement locally without a model-network call", async () => {
    ...
    const baseline = await compileGeneratedProjectFromSemantic({
      ...common,
      requestId: "local-baseline",
      controls: DEFAULT_GENERATED_CONTROLS
    });
    const edited = await compileGeneratedProjectFromSemantic({
      ...common,
