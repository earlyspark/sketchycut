...
import { compileRetainedPinFixture } from "../helpers/retained-pin-fixtures.js";

describe("retained-pin linked projections", () => {
  it("projects one canonical motion and external stock identity through scene, BOM, and instructions but never SVG", async () => {
    const value = await compileRetainedPinFixture("hinged-lid-box");
  ...
  it("preserves every sheet-part ID exactly once across SVG, mesh, BOM, legend, and instructions", async () => {
    const value = await compileRetainedPinFixture("hinged-flap");
    const artifacts = await buildMultiSheetProjectionBundle(
      value.document,
      nestPartsAcrossSheets(value.document.parts, value.profiles.machine, value.profiles.material, value.profiles.processRecipe, value.profiles.fabricationContext),
    );
